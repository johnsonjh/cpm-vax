...
PLoop_1:
 ...
 #	offset to the top of the loop.
	...
	addl2 (r7)+,(sp)	# Add increment to index and discard increment.
 ...
 #	$$$
	...
	code "PDo","(DO)","PPLoop"
	movl 4(r7),-(sp)	# push limit
	movl (r7)+,-(sp)	# push init and discard
	tstl (r7)+		# Discard limit
	jmp (r11)
 ...
 #	Since R is essentially a synonym for I, it enters at RI1.
	...
	code "I","I","LEAVE"
RI1:	movl (sp),-(r7)		# Push a copy of top of return stack
	jmp (r11)
 ...
 #	important to FORTH multitasking.
	...
	movl r7,-(r7)		# Push a copy of the math stack pointer.
	jmp (r11)
...
DDup1:	offset SemiS
 ...
 #	next item on the math stack, returning the difference.
	...
	subl2 (r7)+,(r7)	# Subtract top item and discard
	jmp (r11)
 ...
 #	cle				-- Apologies to Arlo Guthrie
	...
	bicl2 (r7)+,(r7)	# BICL it and discard b
	jmp (r11)
 ...
 #	on the math stack.
	...
	code "OR","OR","AND"
	bisl2 (r7)+,(r7)	# Do it and discard b
	jmp (r11)
 ...
 #	on the math stack.
	...
	code "XOR","XOR","OR"
	xorl2 (r7)+,(r7)	# Do it and discard b
	jmp (r11)
 ...
 #	address specified on top of the math stack.
	...
	code "PlusStore","+!","XOR"
	addl2 4(r7),*(r7)+	# Add val to addr and discard addr
	tstl (r7)+		# DROP val
	jmp (r11)
 ...
 #	norm of making it difficult to do in a single instruction...
	...
	xorl2 (r7)+,*(r7)+	# Toggle bits and discard bits and addr
	jmp (r11)
...
Yes:	movzbl $1,(r7)		# It's true. Push a one.
 ...
 #	$$$
	...
	code "ULess","U<","NotEq"
	cmpl 4(r7),(r7)+	# Compare and discard b
...
Max1:	offset DROP
 ...
 #	Shifts the value left by the specified count
	...
	movl (r7)+,r0		# Get count and discard
	ashl r0,(r7),(r7)	# Do the shift
	jmp (r11)
 ...
 #	Shifts the value right by the specified count
	...
	mnegl (r7)+,r0		# Get count, make it a left shift count, discard
	ashl r0,(r7),(r7)	# Do the shift
	jmp (r11)
 ...
 #	math stack, returning a 64-bit result.
	...
	movl (r7)+,r1		# Get bhi and discard
	movl (r7)+,r0		# Get blo and discard
	addl2 r0,4(r7)		# Add blo to alo
	adwc r1,(r7)		# Add bhi and carry to ahi
	jmp (r11)
 ...
 #	result is returned on the math stack.
	...
	movl (r7)+,r1		# Get bhi and discard from math stack
	movl (r7)+,r0		# Get blo and discard from math stack
	subl2 r0,4(r7)		# Subtract low longword
	sbwc r1,(r7)		# Subtract high longword and borrow
	jmp (r11)
 ...
 #	from zero.
	...
	subl3 4(r7),r0,4(r7)	# Complement low word
...
DPM1:	offset SemiS
 ...
 #	double-length is 64 bits, but we won't be picky.
	...
	code "UStar","U*","DABS"
	emul (r7)+,(r7)+,$0,r0	# Do it
	movl r0,-(r7)		# Push result
	movl r1,-(r7)
	jmp (r11)
 ...
 #	their work and then pick the results they want.
	...
	code "USlash","U/","UStar"
	movl (r7)+,r2		# Denominator
	movl (r7)+,r1		# high-order numerator
	movl (r7)+,r0		# low-order numerator
	ediv r2,r0,r3,r4	# Do it
	movl r4,-(r7)		# Push remainder
	movl r3,-(r7)		# Push quotient
	jmp (r11)
 ...
 #	$$$
	...
	clrl -(r7)		# Assume it's positive
	tstl 4(r7)		# Are we right?
	bgeq S2D1		# If so, go
	decl (r7)		# If not, turn the 0 into a -1
S2D1:	jmp (r11)
 ...
 #	instruction because the words would be stacked in the wrong order.
	...
	movl (r7)+,r0		# Get the address
	movl 4(r0),-(r7)	# Fetch high-order longword
	movl (r0),-(r7)		# Fetch low-order longword
	jmp (r11)
 ...
 #	stack.
	...
	code "Store","!","TFetch"
	movl 4(r7),*(r7)+	# Store value and discard address
	tstl (r7)+		# Discard value
	jmp (r11)
 ...
 #	stack.
	...
	code "CStore","C!","Store"
	movb 4(r7),*(r7)+	# Store value and discard address
	tstl (r7)+		# Discard value
	jmp (r11)
 ...
 #	This procedure stores a quadword from the math stack to memory.
	...
	movq 4(r7),*(r7)+	# Store value and discard address
	addl2 $8,r7		# Discard val1 and val2
	jmp (r11)
...
CMove1:	movb *8(r7),*4(r7)	# Move a byte
	incl 8(r7)		# Advance source
	incl 4(r7)		# Advance dest
	sobgtr (r7),CMove1	# Lather, rinse, and repeat
	addl2 $12,r7		# Toss number, dest, and source
	jmp (r11)
 ...
 #	$$$
	...
	tstl 4(r7)		# Did we get a zero count?
	beql FILL1		# If so, go

				# Now make the stack look good like what
				# CMOVE wants

	movb (r7),*8(r7)	# Prime the pump
	subl3 $1,4(r7),(r7)	# Adjust the count to account for the priming
	addl3 $1,8(r7),4(r7)	# Dest is just after the priming
	jmp CMOVE_pfa		# Shazam!!!

FILL1:	addl2 $12,(r7)		# DROP the arguments
...
vlist2:	offset DROP
 ...
 #	We start off with 4(AP) pointing to the string we're searching
 #	for and (AP) pointing to the place we're supposed to start
 #	looking.
...
PFind1:
 ...
 #	bits off the length byte at Start except for the smudge bit.
	...
	bicb3 $0xe0,*(r7),r0	# Strip the bits
	cmpb r0,*4(r7)		# Do the length bytes match?
	bneq PFind2		# If not, it can't be a match

 #	We have strings of the same length. Now compare the strings

	movl (r7),r4		# Get addresses
	movl 4(r7),r5
	incl r4			# Bump past length byte
	incl r5
	movzbl *4(r7),r9	# Need a word string length
...
0:	cmpb (r4)+,(r5)+	# MicroVAX II doesn't do CMPC3
 ...
 #	The strings matched. Diddle the stack.
	...
	bicb3 $0xe0,*(r7),r1	# Figure out where the PFA is
	movzbl r1,r1
	addl3 r1,(r7),r0	# Here's the LFA
	movzbl *(r7),r1		# Get raw length byte again
	addl3 $9,r0,4(r7)	# There's the PFA, and don't forget the
				# length byte...
	movzbl r1,(r7)		# And the length
...
PFind2:
	...
	bicb3 $0xe0,*(r7),r0	# Get length of string
	movzbl r0,r0
	addl2 r0,(r7)		# Make address of LFA
	incl (r7)		# (account for the length byte)
	movl *(r7),r0		# Get link field value
	tstl r0			# Is it the end flag?
	beql PFind3
	addl2 r0,(r7)		# Update the start address
...
PFind3:
	...
	tstl (r7)+		# Discard starting address
	clrl (r7)		# Signal failure
	jmp (r11)
...
#	Calls the BDOS, pushing the return value.
	...
	movl 4(r7),-(sp)	# Push parameter
	movl (r7)+,-(sp)	# Push func and pop it
